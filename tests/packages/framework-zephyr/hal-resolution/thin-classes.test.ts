...
describe('thin GPIO lowering', () => {
  ...
  it('configure on a dtSpec pin is guarded (once) and uses gpio_pin_configure_dt', () => {
    // XIAO led0 = pin 26.
    const out = lowerGpio({ operation: 'gpio.configure', pin: 26, flags: 'GPIO.OUTPUT | GPIO.OUTPUT_INIT_LOW' } as any, XIAO_BLE);
...
describe('thin PWM lowering', () => {
  it('set_pulse applies the construction period once, then pwm_set_pulse_dt', () => {
    const out = lowerPwm({ operation: 'pwm.set_pulse', pin: 17, periodNs: 20000000, pulseNs: 1500000 } as any, XIAO_BLE);
  ...
  it('set_duty scales the fraction 0.0–1.0 against the construction period (one call)', () => {
    const out = lowerPwm({ operation: 'pwm.set_duty', pin: 17, periodNs: 20000000, duty: 0.75 } as any, XIAO_BLE);
  ...
  it('set_period lowers to pwm_set_dt with an idle pulse (no period-only API)', () => {
    const out = lowerPwm({ operation: 'pwm.set_period', pin: 17, periodNs: 50000000 } as any, XIAO_BLE);
...
describe('thin ADC lowering', () => {
  it('read_raw does lazy inline channel setup then adc_read', () => {
    ...
    const out = lowerAdc({ operation: 'adc.read_raw', pin: 2, gain: '', reference: '' } as any, XIAO_BLE);
  ...
  it('read_raw honors construction gain/reference tokens over the descriptor', () => {
    const out = lowerAdc({ operation: 'adc.read_raw', pin: 2, gain: 'ADCChannel.GAIN_1_6', reference: 'ADCChannel.REF_VDD_1' } as any, XIAO_BLE);
  ...
  it('read_mv converts via adc_raw_to_millivolts with the setup gain', () => {
    const out = lowerAdc({ operation: 'adc.read_mv', pin: 2, gain: 'ADCChannel.GAIN_1', reference: '' } as any, XIAO_BLE);
  ...
  it('unknown gain tokens are build errors naming the valid spellings', () => {
    expect(() =>
      lowerAdc({ operation: 'adc.read_raw', pin: 2, gain: 'ADCChannel.GAIN_QUARTER', reference: '' } as any, XIAO_BLE),
    ).toThrow(/GAIN_QUARTER.*ADCChannel\.GAIN_1_4/s);
  });
});
...
describe('thin Watchdog lowering', () => {
  it('setup installs the construction timeout (ms) and arms — no WDTO parsing', () => {
    const out = lowerWdt({ operation: 'wdt.setup', timeoutMs: 2500 } as any, XIAO_BLE);
  ...
  it('feed lowers to wdt_feed on the installed channel', () => {
    const out = lowerWdt({ operation: 'wdt.feed' } as any, XIAO_BLE);
...
describe('thin Counter lowering', () => {
  it('on_alarm stores the handler on the instance callback var', () => {
    const out = lowerCounter({ operation: 'counter.on_alarm', instance: 0, handler: 'tick' } as any, XIAO_BLE);
    expect(out.code).toBe('__tc_hw_cb_0 = (tick);');
  });

  it('start applies the construction hz as the top value and starts', () => {
    const out = lowerCounter({ operation: 'counter.start', instance: 0, hz: 1000 } as any, XIAO_BLE);
  ...
  it('stop halts the counter', () => {
    const out = lowerCounter({ operation: 'counter.stop', instance: 0 } as any, XIAO_BLE);
    expect(out.code).toBe('(void)counter_stop(__tc_hw_dev_0);');
  });

  it('out-of-range instances lower to a comment naming the count', () => {
    const out = lowerCounter({ operation: 'counter.start', instance: 3, hz: 10 } as any, XIAO_BLE);
