...
describe('RP2040 (Pico) Zephyr emit', () => {
  it('lowers ADC.read on GP26 (capability check via flattened adc functions)', () => {
    const result = transpile(`
      import { ADC } from '@typecad/hal';
      const sense = new ADC(26); // GP26
      const v: number = sense.read();
      console.log('v' + v);
    `, rp2040);

    // Silicon facts (ADC channels) never lived in devicetree — the equal
    // path honestly reports the missing capability instead of lowering.
    expect(result.cpp).not.toContain('DT_NODELABEL(adc)');
    // No ADC channel setup is emitted for an equal-path chip with no
    // silicon ADC facts.
    expect(result.cpp).not.toContain('__tc_adc0_setup');
  });
...
describe('RP2350 (Pico 2) Zephyr emit', () => {
  it('lowers a thin-GPIO LED toggle and an ADC read in the same program', () => {
    const result = transpile(`
      const GP25 = 25;
const GP26 = 26;
      import { GPIO, ADC } from '@typecad/hal';
