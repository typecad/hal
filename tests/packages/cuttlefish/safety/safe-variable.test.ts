...
`;
  ...
  it("does NOT emit SafeVariable template when the program doesn't use it", () => {
    // Polyfill emission is gated on programUsesSafety(program), which detects
    // SafeVariable/SafeInt usage via var_decl cppTypes. A program with no safety
    ...
    // *this` chaining methods injected a `this` token into every program).
    const ts = `const x = 5;`;
