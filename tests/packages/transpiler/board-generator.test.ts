...
describe('board-generator', () => {
  it('should generate a forwarding board file when no connected pins are provided', () => {
    ...
    const outPath = generateBoardFile(dir, '@typehal/board-arduino-uno');
    
    expect(outPath).toBe(path.join(dir, '.typehal', 'board.ts'));
  ...
  it('should generate a narrowed board file when connected pins are provided', () => {
    ...
    const outPath = generateBoardFile(dir, '@typehal/mcu-atmega328p', connectedPins);
