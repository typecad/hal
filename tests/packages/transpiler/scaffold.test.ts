// ---------------------------------------------------------------------------
// Tests for `cuttlefish create` project scaffolding
// ---------------------------------------------------------------------------
...
const ARDUINO_UNO_OPTIONS: CreateProjectOptions = {
...
describe("create templates", () => {
  describe("generateProjectPackageJson", () => {
    ...
    it("omits both test tiers for native targets", () => {
      ...
      const nativeOptions: CreateProjectOptions = {
  ...
  describe("generateProjectConfig", () => {
    ...
    it("expands the esp32s3 registry mcu to @typecad/mcu-esp32s3", () => {
      ...
      const s3Options: CreateProjectOptions = {
        projectName: 'test-project',
        boardId: s3Target!.id,
...
describe("create scaffold", () => {
  ...
  describe("KNOWN_BOARDS", () => {
    ...
    it("contains xiao-nrf52840", () => {
      const b = KNOWN_BOARDS.find(b => b.id === 'xiao-nrf52840');
      expect(b).toBeDefined();
      expect(b!.architecture).toBe('nrf52');
      // Zephyr-only target: the build target is the west board id, not an FQBN.
      expect(b!.buildTarget).toBe('xiao_ble/nrf52840');
    });
  });
  ...
  describe("scaffoldProject", () => {
    ...
    beforeEach(() => {
      tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'cuttlefish-create-test-'));
    });
