...
// tree into <workspace>/.typecad-hal/board-catalog.json. There is no
...
function resolveCatalog(): void {
  ...
  if (ensure.status === 'no-overlay') {
    console.error(`No board catalog overlay at ${ensure.overlayPath}. Run: typecad-hal board sync`);
    process.exit(1);
  }
  const boards = activeBoardCatalog();
  if (Object.keys(boards).length === 0) {
    console.error('The board catalog resolved to zero boards — re-run: typecad-hal board sync');
    process.exit(1);
  }
  CATALOG = boards;
  CATALOG_INFO = {
...
function initMasterHarness(opts: Options): string {
  ...
  if (!existsSync(join(master, 'package.json'))) {
    ...
    if (created.code !== 0) {
      console.error(`typecad-hal create failed:\n${created.output.slice(-2000)}`);
      process.exit(1);
    }
  }
  ...
  // The testing DSL and the simulator both live inside @typecad/hal now —
  // rewiring hal to the workspace checkout covers them.
...
async function runBoard(harness: string, opts: Options, target: string, perBuildJobs?: number): Promise<BoardResult> {
  ...
  writeFileSync(join(harness, 'typecad-hal.config.ts'), configFor(target));
  ...
  rmSync(join(harness, '.typecad-hal', 'board.json'), { force: true });
  rmSync(join(harness, '.typecad-hal', 'board.ts'), { force: true });
