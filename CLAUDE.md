...
### Lowering / emission
- **Nullish coalescing** (`??`) must lower via `typecode_nullish` helper — never a truthy ternary — so `0` and `false` are preserved as values.
...
- The `typecode_nullish` helper signature is `template <typename T, typename U> inline T typecode_nullish(T value, U fallback)` so mixed types (e.g. `uint8_t` fallback) compile cleanly.
...
### Framework changes
- Changes under `packages/framework-arduino` are consumed through built package output during transpile. Rebuild that package **and** the CLI after any `strategy.ts` / `typecode-map.ts` edits.
