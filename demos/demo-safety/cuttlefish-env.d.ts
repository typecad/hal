...
declare global {
  ...
  // SafeVariable: SEU-resistant storage. The transpiler lowers this to a
  // C++ template that stores value + bitwise inverse, validating on read.
  // Declared as an interface (not a type alias) so the TS type checker
  // recognizes .set()/.get()/.valid() method calls.
  interface SafeVariable<T = number | string> {
    set(value: T): void;
    get(): T;
    valid(): boolean;
  }
