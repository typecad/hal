...
void setup()
{
  const auto n = __tc_wifi_scan();
  char __cuttlefish_str_1[28];
  snprintf(__cuttlefish_str_1, sizeof(__cuttlefish_str_1), "found %d networks", n);
  printf("%s\n", __cuttlefish_str_1);
  for (int i = 0; i < n; i++)
  {
    char __cuttlefish_str_2[181];
    snprintf(__cuttlefish_str_2, sizeof(__cuttlefish_str_2), "%s  %d dBm  ch%d  enc=%d", __tc_wifi_scan_ssid(i), __tc_wifi_scan_rssi(i), __tc_wifi_scan_channel(i), __tc_wifi_scan_encryption(i));
    printf("%s\n", __cuttlefish_str_2);
  }
  while (true)
  {
