...
// 02 — configured connect: hostname, static IP, power-save off, custom
...
{
  {
    __tc_wifi_static_ip("192.168.1.50", "192.168.1.1", "255.255.255.0", "1.1.1.1");
    __tc_wifi_set_power_save("none");
  }
  if (!__tc_wifi_connect(WIFI_SSID, WIFI_PASSWORD, 30000))
  {
    ESP_LOGE("tc", "[ERROR] connect failed");
  }
  else {
    printf("%s\n", __tc_wifi_local_ip());
    printf("%s\n", __tc_wifi_mac());
