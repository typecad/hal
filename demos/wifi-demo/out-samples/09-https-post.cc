...
void setup()
{
  ...
  __tc_http_reset();
  {
    char __cuttlefish_snprintf_0[35];
    snprintf(__cuttlefish_snprintf_0, sizeof(__cuttlefish_snprintf_0), "{\\\"temp\\\":21.5,\\\"rssi\\\":%d}", __tc_wifi_rssi());
    __tc_http_set_body(__cuttlefish_snprintf_0, true);
  }
  {
    __tc_http_begin(HTTP_METHOD_POST, "https://httpbin.org/post");
    __tc_http_send();
  }
  printf("%s\n", __tc_http_body());
  while (true)
