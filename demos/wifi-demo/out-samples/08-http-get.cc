...
void setup()
{
  ...
  __tc_http_reset();
  __tc_http_set_timeout(10000);
  {
    __tc_http_begin(HTTP_METHOD_GET, "http://httpbin.org/get");
    __tc_http_send();
  }
  char __cuttlefish_str_1[20];
  snprintf(__cuttlefish_str_1, sizeof(__cuttlefish_str_1), "status=%d", __tc_http_status());
  printf("%s\n", __cuttlefish_str_1);
  printf("%s\n", __tc_http_body());
  while (true)
  {
