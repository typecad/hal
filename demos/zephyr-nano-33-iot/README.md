...
this target — see the generated board module (`.typecad-hal/board.ts`) for
the full capability notes.

```ts
import { PWMLED, USB0 } from '@typecad/hal';
...
npm run test:hw        # flash + run tests/ on the board (@typecad/hal/testing)
```
...
**Bootloader entry (bossac only):** normally automatic — typecad-hal's CDC
...
double-tap reset button is only needed if the board is running non-typecad-hal
...
| `2FE3:0001` | TypeCAD firmware running (touch-capable, default CDC PID) |

Override the serial port with `--port` (the config's `COM8` default is
bring-up specific). After the first typecad-hal flash the board enumerates
at the Zephyr-test default `2FE3:0001` (shared by every Zephyr CDC board);
set `test.usb: { vid: '2FE3', pid: '0001' }` in typecad-hal.config.ts to let
identity matching track the COM number across re-enumerations.

## Layout

- `src/main.ts` — the firmware (transpiled to C++ by typecad-hal)
