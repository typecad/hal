...
{
  {
    __tc_ble_set_name("MultiSensor");
    __tc_ble_add_char(0, "2A6E", "int16", 5, 1);
    __tc_ble.on_read[0] = &main_isr_0;
  }
  {
    __tc_ble_set_name("MultiSensor");
    __tc_ble_add_char(1, "2A6F", "uint16", 5, 1);
    __tc_ble.on_read[1] = &main_isr_1;
  }
  {
    __tc_ble_set_name("MultiSensor");
    __tc_ble_add_char(2, "2A19", "uint8", 5, 1);
    __tc_ble.on_read[2] = &main_isr_2;
  }
  {
    __tc_ble_set_name("MultiSensor");
    __tc_ble_server_begin("MultiSensor");
    __tc_ble_advertise_start();
  }
  printf("advertising 3 characteristics\n");
  while (true)
  {
    __tc_delay(5000);
    batteryPct = (batteryPct > 20 ? batteryPct - 1 : 100);
    char __cuttlefish_str_1[22];
    snprintf(__cuttlefish_str_1, sizeof(__cuttlefish_str_1), "battery=%d%%", batteryPct);
    printf("%s\n", __cuttlefish_str_1);
  }
}
