...
{
  {
    __tc_ble_set_name("StatusDemo");
    __tc_ble_add_char(0, "2A6E", "int16", 1, 1);
    __tc_ble.on_read[0] = &main_isr_0;
  }
  {
    __tc_ble_set_name("StatusDemo");
    __tc_ble_server_begin("StatusDemo");
    __tc_ble_advertise_start();
  }
  while (true)
  {
    const auto connected = __tc_ble_is_connected();
    if (connected != wasConnected)
    {
      wasConnected = connected;
      printf("%g\n", (connected ? "central connected" : "central disconnected"));
      char __cuttlefish_str_1[22];
      snprintf(__cuttlefish_str_1, sizeof(__cuttlefish_str_1), "clients: %d", __tc_ble_client_count());
      printf("%s\n", __cuttlefish_str_1);
    }
    if (__tc_ble.status == BleStatus.Error)
    {
      printf("BLE error\n");
    }
    __tc_delay(500);
