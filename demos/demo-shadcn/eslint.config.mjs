...
const transpilerRules = [
  ...
  // C++ equivalent — the transpiler rejects them ("no C++ equivalent"), and
  ...
  // ────────────────────────────────────────────────────────────────────────
  // Lint-derived selectors. Each rule below maps a ❌
  ...
  // §2.6 🟡 — generators are parse-compatible but cannot produce
  // correct firmware behavior; treat as unsupported to avoid silent surprises.
  // (async/await is fully supported — lowered to a cooperative state machine;
  // the stale async selectors that used to sit here were false positives.)
  {
  ...
  // that an AOT C++ lowering cannot model (RTTI, JS runtime, dynamic shape).
  // ────────────────────────────────────────────────────────────────────────
