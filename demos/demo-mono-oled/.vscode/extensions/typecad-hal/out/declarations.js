...
function registerDeclarations(context, resolveRoot) {
...
class DeclarationGenerator {
    ...
    async checkAndGenerate(cppPath) {
        ...
        if (result) {
            void vscode.window.showInformationMessage(`typeCAD/hal: Generated ${path.basename(declPath)} from ${path.basename(cppPath)}. Review and adjust types if needed.`);
        }
    }
    /**
    ...
    async generateDeclaration(cppPath, declPath) {
        return new Promise((resolve) => {
            ...
            (0, node_child_process_1.exec)(cmd, { cwd: root }, (error) => {
                if (error) {
