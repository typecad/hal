...
This guide walks through creating a complete board package, using the two existing boards — `board-arduino-uno` and `board-esp32-devkit` — as reference.
...
After building, the new board package is available to any `.ts` file in the workspace:

```typescript
import { Board, LED, delay } from '@typecode/board-rp2040-pico';
...
- [ ] Test sketch compiles for the target FQBN
