...
static inline uint8_t ui_render_screen_bands(int16_t rx, int16_t ry, int16_t rw, int16_t rh) {
  ...
  for (int16_t bandTop = 0; bandTop < rh; bandTop += bandH) {
    ...
    for (uint16_t ci = 0; ci < __ui_scroll_candidate_count; ci++) {
      ...
      // faces drop out here.
      {
        int16_t clipTopL = static_cast<int16_t>(__ui_scroll_candidates[ci].clipTop - ry - bandTop);
        int16_t clipBotL = static_cast<int16_t>(__ui_scroll_candidates[ci].clipBottom - ry - bandTop);
        int16_t clipLeftL = static_cast<int16_t>(__ui_scroll_candidates[ci].clipLeft - rx);
        int16_t clipRightL = static_cast<int16_t>(__ui_scroll_candidates[ci].clipRight - rx);
        int16_t boxY0 = __ui_nodes[c].box.y;
        if (boxY0 < clipTopL) {
          __ui_nodes[c].box.y = clipTopL;
          __ui_nodes[c].box.h = static_cast<int16_t>(__ui_nodes[c].box.h - (clipTopL - boxY0));
        }
        if (static_cast<int16_t>(__ui_nodes[c].box.y + __ui_nodes[c].box.h) > clipBotL) {
          __ui_nodes[c].box.h = static_cast<int16_t>(clipBotL - __ui_nodes[c].box.y);
        }
        int16_t boxX0 = __ui_nodes[c].box.x;
        if (boxX0 < clipLeftL) {
          __ui_nodes[c].box.x = clipLeftL;
          __ui_nodes[c].box.w = static_cast<int16_t>(__ui_nodes[c].box.w - (clipLeftL - boxX0));
        }
        if (static_cast<int16_t>(__ui_nodes[c].box.x + __ui_nodes[c].box.w) > clipRightL) {
