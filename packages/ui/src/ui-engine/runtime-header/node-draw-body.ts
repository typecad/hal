      ...
      case NODE_TEXT:
        {
          ...
          }
          {
            ...
            // Incremental repaint path: the clear wipes this node's previous
            // glyphs (and stale paint wider/taller than the box). On the mono
            // full-frame path the whole frame is re-seeded by display_fillScreen
            // and every node repaints in z order, so the clear is redundant —
            // and worse, it ERASES previous siblings' descenders that dip into
            // this box (the ladder's cut g tails).
...
#else
            (void)insetL; (void)insetR; (void)insetT; (void)textW; (void)textBoxPainted;
#endif
