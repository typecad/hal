...
static inline const UIFontFace* ui_font_face(uint8_t id) {
 ...
 static inline int8_t ui_font_glyph_index(const UIFontFace* face, uint16_t codepoint) {
   if (!face) return -1;
  for (uint8_t i = 0; i < face->glyphCount; i++) {
    if (face->glyphs[i].codepoint == codepoint) return static_cast<int8_t>(i);
  }
  return -1;
}

static inline const UIFontGlyph* ui_font_glyph(const UIFontFace* face, uint16_t codepoint) {
  const int8_t idx = ui_font_glyph_index(face, codepoint);
  return idx >= 0 ? &face->glyphs[static_cast<uint8_t>(idx)] : nullptr;
}

// Kerning adjustment between two glyphs by subset-local index. The pair
// table is (left, right)-sorted at emit time, so this is a binary search.
static inline int8_t ui_font_kern(const UIFontFace* face, int8_t leftIdx, int8_t rightIdx) {
  if (!face || !face->kern || face->kernCount == 0 || leftIdx < 0 || rightIdx < 0) return 0;
  int16_t lo = 0;
  int16_t hi = static_cast<int16_t>(face->kernCount - 1);
  const uint8_t l = static_cast<uint8_t>(leftIdx);
  const uint8_t r = static_cast<uint8_t>(rightIdx);
  while (lo <= hi) {
    const int16_t mid = static_cast<int16_t>(lo + ((hi - lo) >> 1));
    const UIFontKern* k = &face->kern[static_cast<uint8_t>(mid)];
    if (k->left == l && k->right == r) return k->value;
    if (k->left < l || (k->left == l && k->right < r)) lo = static_cast<int16_t>(mid + 1);
    else hi = static_cast<int16_t>(mid - 1);
  }
  return 0;
}
...
static inline uint16_t ui_asset_text_width(const char* text, const UIFontFace* face) {
  ...
  while (*p) {
    ...
    int8_t idx = ui_font_glyph_index(face, codepoint);
    const UIFontGlyph* glyph = idx >= 0 ? &face->glyphs[static_cast<uint8_t>(idx)] : nullptr;
    w += glyph ? glyph->advance : (face->lineHeight / 2);
    // Pair kerning — must match the draw loop exactly (layout parity).
    w = static_cast<uint16_t>(static_cast<int32_t>(w) + ui_font_kern(face, prevIdx, idx));
    prevIdx = idx;
  }
  return w;
}
...
static inline uint16_t ui_text_span_width(const char* start, const char* end, uint8_t ts, uint8_t fontFace, int8_t letterSpacing) {
  ...
  while (p < limit && *p) {
    ...
    if (face) {
      // Asset face: advance + pair kerning, matching the draw loop. (The
      // generic advance below can't kern — it sees one codepoint at a time.)
      int8_t idx = ui_font_glyph_index(face, codepoint);
      const UIFontGlyph* glyph = idx >= 0 ? &face->glyphs[static_cast<uint8_t>(idx)] : nullptr;
      w += glyph ? glyph->advance : (face->lineHeight / 2);
      w = static_cast<uint16_t>(static_cast<int32_t>(w) + ui_font_kern(face, prevIdx, idx));
...
static inline uint8_t ui_draw_asset_text(const char* text, int16_t x, int16_t y, UI_COLOR_T fg, UI_COLOR_T bg, uint8_t antialias, uint8_t fontFace) {
  ...
  while (*p) {
    ...
    if (glyphX + static_cast<int16_t>(glyph->width) <= targetLeft || glyphX >= targetRight ||
      ...
      cursor = static_cast<int16_t>(cursor + ui_font_kern(face, prevIdx, idx));
      prevIdx = idx;
      continue;
    }
