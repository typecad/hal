...
export interface UIImageAsset {
  ...
  data: number[];
  /** Mono flattening mode for this image (from <img dither="...">; default threshold). */
  dither?: "threshold" | "floyd-steinberg";
}

// monoImageBits + MonoImageMode live in image-mono.ts (browser-safe leaf —
// the preview imports them, and this module carries node:fs). Re-exported
// here so bake-side importers are unchanged.
export { monoImageBits } from "./image-mono.js";
export type { MonoImageMode } from "./image-mono.js";
import { monoImageBits } from "./image-mono.js";
  ...
  const walk = (node: StyledNode) => {
    ...
    if (src && node.id) {
      ...
      if (assetIdx === undefined) {
        ...
        if (asset) {
          asset.dither = (node as any).imgDither;
...
export function emitImageTables(assets: UIImageAsset[], colorFormat: ColorFormat = "rgb565"): string {
  ...
  for (const asset of assets) {
    if (mono) {
      // 1bpp packed — see monoImageBits for the layout/threshold contract.
      const bits = monoImageBits(asset, asset.dither === "floyd-steinberg" ? "floyd-steinberg" : "threshold");
      lines.push(`static const uint8_t __ui_img_${asset.id}_data[] = {`);
      for (let i = 0; i < bits.length; i += 16) {
        lines.push("  " + bits.slice(i, i + 16).map((v) => `0x${(v & 0xff).toString(16).padStart(2, "0")}`).join(", ") + ",");
      }
      lines.push("};");
    } else {
