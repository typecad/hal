...
//
// All debug code generation lives in the CLI; no framework package dependency.
// ---------------------------------------------------------------------------
...
function generateBreakpointCode(
  ...
  if (bp.logMessage) {
    ...
    return generateLogpointCode(fileName, lineNum, parts, variables);
  }
  ...
  return generateBreakpointSerialCode(fileName, lineNum, originalLine, variables, normalizedCondition);
}

// ---------------------------------------------------------------------------
// Serial debug code generation (inlined from framework-arduino)
// ---------------------------------------------------------------------------
 ...
 * Generate the Serial debug code for a breakpoint.
 */
function generateBreakpointSerialCode(
  fileName: string,
  lineNum: number,
  originalLine: string,
  variables: CapturedVariable[],
