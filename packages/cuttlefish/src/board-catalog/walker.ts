...
function composeRecord(base: BoardDataEntry, facts: ReturnType<typeof readBoardDts>, zephyrBase: string, westRoot: string): { record: BoardDataEntry; hasFacts: boolean } {
  ...
  const rec: Record<string, unknown> = {
    ...
    ...(facts.powerStates && facts.powerStates.length > 0 ? { powerStates: facts.powerStates } : {}),
    ...(facts.rtcWakeTimer ? { rtcWakeTimer: true } : {}),
    ...(facts.canNodes.length > 0 ? { canNodes: facts.canNodes } : {}),
    ...(facts.i2sNodes.length > 0 ? { i2sNodes: facts.i2sNodes } : {}),
