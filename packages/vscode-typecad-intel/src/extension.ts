// ---------------------------------------------------------------------------
// TypeCAD Intel — board-aware language intelligence, vendored into every
// typecad-hal project's .vscode/extensions/ by `typecad-hal create` (staged
// from packages/cuttlefish/assets/editor-extensions/typecad-intel).
//
// The extension is deliberately THIN. It resolves the PROJECT's own
...
import {
  ...
  isCodePosition,
  type BoardFacts,
  type SensorPartInfoLike,
} from './board-facts';

/** Structural slice of @typecad/cuttlefish's ./language-server surface. */
interface EngineModule {
  analyzeForEditor(opts: { workspaceRoot: string }): Promise<EngineAnalysis>;
  listPorts(): Promise<EditorSerialPort[]>;
}

interface EditorSerialPort {
  path: string;
  vid: string;
  pid: string;
  serialNumber?: string;
...
interface EngineAnalysis {
  ...
  pinUsage: { pinName: string; mode: string; peripheralRole?: string }[];
  summary: {
    staticBytes: number;
    stackDepth: number;
    asyncTasks: number;
...
/** The engine's Diagnostic (1-based line/column, same shape the CLI prints). */
interface EngineDiagnostic {
  ...
  source?: string;
  filePath?: string;
  /** A mechanical fix the engine computed — drives quick-fixes. */
  fix?: {
    title?: string;
    swap?: { from: string; to: string };
    edits?: Array<{ line: number; column: number; fromText: string; toText: string }>;
  };
}
...
let boardFacts: BoardFacts | undefined;
/** The sensor part catalog from the project's hal copy (lazy, best-effort). */
let sensorCatalog: Readonly<Record<string, SensorPartInfoLike>> | undefined;
/** The last analysis's program facts — pin claims + headline numbers. */
let lastAnalysis: Pick<EngineAnalysis, 'pinUsage' | 'summary'> | undefined;
/** Quick-fix payloads keyed by the published diagnostic they belong to. */
const fixes = new Map<
  vscode.Diagnostic,
  NonNullable<EngineDiagnostic['fix']>
>();
/** The after-line fact chips' decoration type (theme-styled, per-line text). */
let chipDecoration: vscode.TextEditorDecorationType;
/** Shared terminal for the flash/test commands ("TypeCAD"). */
let terminal: vscode.Terminal | undefined;
/** The port chosen this session (picked once, reused). */
let chosenPort: string | undefined;
/** The last analysis's board label, for status-line rebuilds. */
let lastBoardName: string | undefined;
...
export function activate(context: vscode.ExtensionContext): void {
  ...
  status.text = '$(circuit-board) TypeCAD';
  status.tooltip = 'TypeCAD Intel — click to re-analyze the project';
  status.show();
  ...
  context.subscriptions.push(
    ...
    collection,
    chipDecoration,
    vscode.commands.registerCommand('typecad-intel.reanalyze', () => run()),
    ...
    vscode.commands.registerCommand('typecad-intel.fixApplied', (uri: vscode.Uri, identity: { line: number; code?: string; message: string }) => {
      const current = collection.get(uri);
      if (!current) return;
      const kept = current.filter(
        (d) =>
          !(
            d.range.start.line === identity.line
            && String(d.code ?? '') === String(identity.code ?? '')
            && d.message === identity.message
          ),
      );
      if (kept.length !== current.length) collection.set(uri, kept);
      refreshStatusFromCollection();
    }),
    vscode.commands.registerCommand('typecad-intel.flashMonitor', (uri?: vscode.Uri) => runFlashMonitor(uri)),
    vscode.commands.registerCommand('typecad-intel.runTests', (uri?: vscode.Uri) => runHardwareTests(uri)),
    ...
    vscode.languages.registerCodeLensProvider(
      [{ language: 'typescript' }, { language: 'javascript' }, { language: 'typecad-ui' }],
      { provideCodeLenses, resolveCodeLens },
    ),
    vscode.window.onDidChangeActiveTextEditor(() => updateDecorations()),
    vscode.workspace.onDidChangeTextDocument(() => scheduleDecorations()),
    vscode.workspace.onDidSaveTextDocument((doc) => {
      const lang = doc.languageId;
      if (lang === 'typescript' || lang === 'javascript' || doc.fileName.endsWith('.ui')) schedule();
    }),
    watch('**/typecad-hal.config.ts'),
    watch('**/typecad-hal.facts.json'),
    watch('**/.typecad-hal/board.json'),
    watch('**/.typecad-hal/board.ts', () => {
...
function watch(glob: string, onChangeExtra?: () => void): vscode.FileSystemWatcher {
...
async function performAnalysis(): Promise<void> {
  ...
  try {
    ...
    status.text = '$(alert) TypeCAD Intel';
    status.tooltip = `TypeCAD Intel: ${(err as Error).message}\nClick to retry after installing.`;
    return;
  }
  ...
  try {
    ...
    collection.clear();
    status.text = '$(alert) TypeCAD Intel';
    status.tooltip = firstLine((err as Error).message);
    return;
  }
  ...
  for (const d of result.diagnostics ?? []) {
    ...
    if (!entry) continue;
    if (d.fix) fixes.set(entry[1], d.fix);
    const key = entry[0].toString();
    const list = byUri.get(key);
    if (list) list.push(entry[1]);
    else byUri.set(key, [entry[1]]);
  }
  collection.set([...byUri.entries()].map(([key, list]) => [vscode.Uri.parse(key), list] as [vscode.Uri, vscode.Diagnostic[]]));

  const board = result.board ?? 'no board';
  lastBoardName = result.board;
  status.text = `$(circuit-board) ${board} · ${errors}E ${warnings}W`;
  status.tooltip = `TypeCAD Intel — analyzed ${path.basename(result.entryFile)}`
...
function statusSummaryLine(s: EngineAnalysis['summary']): string {
  if (!s || (s.staticBytes === 0 && s.asyncTasks === 0 && s.isrHandlers === 0 && !s.usesTimers)) return '';
  const parts = [`~${s.staticBytes} B static`, `stack depth ${s.stackDepth}`];
  if (s.asyncTasks > 0) parts.push(`${s.asyncTasks} async task${s.asyncTasks === 1 ? '' : 's'}`);
  if (s.isrHandlers > 0) parts.push(`${s.isrHandlers} ISR`);
...
function refreshStatusFromCollection(): void {
  let errors = 0;
  let warnings = 0;
  ...
  status.text = `$(circuit-board) ${lastBoardName ?? 'TypeCAD'} · ${errors}E ${warnings}W`;
}
...
async function loadEngine(): Promise<EngineModule> {
  if (!enginePromise) {
    enginePromise = (async () => {
      ...
      if (!entry) {
        throw new Error(
          '@typecad/cuttlefish not found in this workspace — run npm install, then re-analyze.',
        );
      }
...
function resolveEngineEntry(projectRoot: string): string | undefined {
  try {
  ...
  const direct = path.join(
    projectRoot, 'node_modules', '@typecad', 'cuttlefish', 'dist', 'language-server.js',
  );
...
function rangeOf(d: EngineDiagnostic): vscode.Range {
...
function resolveDiagnosticFile(filePath: string | undefined, entryFile: string): { uri?: vscode.Uri } {
  if (!filePath) return { uri: vscode.Uri.file(entryFile) };
  if (path.isAbsolute(filePath) && fs.existsSync(filePath)) return { uri: vscode.Uri.file(filePath) };
  const resolved = path.resolve(root!, filePath);
  if (fs.existsSync(resolved)) return { uri: vscode.Uri.file(resolved) };
  ...
  if (fs.existsSync(beside)) return { uri: vscode.Uri.file(beside) };
  return {};
}
...
function provideHover(document: vscode.TextDocument, position: vscode.Position): vscode.Hover | undefined {
  if (!boardFacts) return undefined;
  ...
  const inCode = isCodePosition(document.getText(), document.offsetAt(position));
  if (!inCode && !sensorCatalog?.[identifier]) return undefined;
  const info = resolveIdentifierHover(document.getText(), boardFacts, identifier, {
  ...
  if (info.detail) {
    for (const line of info.detail.split('\n')) {
      if (line.trim()) md.appendMarkdown(`\n\n${escapeMarkdown(line)}`);
    }
  }
...
function provideCodeActions(document: vscode.TextDocument, range: vscode.Range): vscode.CodeAction[] {
  const actions: vscode.CodeAction[] = [];
  const diags = collection.get(document.uri) ?? [];
  for (const diag of diags) {
    if (!diag.range.contains(range.start) && !range.contains(diag.range.start)) continue;
    ...
    const action = new vscode.CodeAction(
      fix.title
        ?? (fix.swap ? `TypeCAD: Use ${fix.swap.to} instead of ${fix.swap.from}` : 'TypeCAD: Apply suggested fix'),
 ...
 */
function positionedEdits(
  document: vscode.TextDocument,
  fix: NonNullable<EngineDiagnostic['fix']>,
): vscode.TextEdit[] {
  const out: vscode.TextEdit[] = [];
  for (const e of fix.edits ?? []) {
    if (e.line < 1 || e.line > document.lineCount) continue;
    ...
    if (document.getText(editRange) !== e.fromText) continue;
  ...
  for (const m of text.matchAll(re)) {
    ...
    edits.push(
      vscode.TextEdit.replace(
        new vscode.Range(document.positionAt(start), document.positionAt(start + fix.swap.from.length)),
        fix.swap.to,
      ),
    );
  }
...
function scheduleDecorations(): void {
  if (decorationTimer !== undefined) clearTimeout(decorationTimer);
...
function updateDecorations(): void {
  for (const editor of vscode.window.visibleTextEditors) {
    ...
    const specs = scanDecorations(editor.document.getText(), boardFacts);
...
function resolveCodeLens(lens: vscode.CodeLens): vscode.CodeLens {
  return lens;
}
...
/** Ask for the serial port once per session (quick pick, then manual entry). */
async function pickPort(): Promise<string | undefined> {
  if (chosenPort) return chosenPort;
  let ports: EditorSerialPort[] = [];
  try {
    const engine = await loadEngine();
    ports = await engine.listPorts();
  } catch {
    // no engine/serialport — straight to manual entry
  }
  if (ports.length > 0) {
    const pick = await vscode.window.showQuickPick(
      ...
      { placeHolder: 'Select the board’s serial port' },
    );
    if (pick) chosenPort = pick.value;
    return chosenPort;
  }
  ...
  if (typed) chosenPort = typed.trim();
  return chosenPort;
}

function typecadTerminal(): vscode.Terminal {
...
async function runFlashMonitor(uri?: vscode.Uri): Promise<void> {
  ...
  if (!port) return;
  typecadTerminal().sendText(`npx typecad-hal build --compile --upload --monitor --port ${port}`);
}

async function runHardwareTests(uri?: vscode.Uri): Promise<void> {
  if (!root) return;
  ...
  typecadTerminal().sendText(`npx typecad-hal test "${rel}" --port ${port}`);
}
 ...
 */
function loadBoardFacts(): BoardFacts | undefined {
  if (!root) return undefined;
  let dir: string | undefined = root;
  for (let depth = 0; depth < 8 && dir; depth++) {
    ...
    try {
      if (fs.existsSync(boardTs)) {
        ...
        return parseBoardModule(fs.readFileSync(boardTs, 'utf8'), jsonText);
      }
    } catch {
      return undefined;
    }
 ...
 */
async function loadSensorCatalog(): Promise<void> {
  if (!root) return;
  try {
    const req = createRequire(path.join(root, 'package.json'));
    const entry = req.resolve('@typecad/hal/core');
    const url = 'file:///' + path.resolve(entry).split(path.sep).join('/').replace(/^\/+/, '');
    const mod = (await nativeImport(url)) as { SENSOR_PART_INFO?: Readonly<Record<string, SensorPartInfoLike>> };
    if (mod.SENSOR_PART_INFO) sensorCatalog = mod.SENSOR_PART_INFO;
  } catch {
