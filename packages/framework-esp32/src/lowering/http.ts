...
export function httpInitLines(): string[] {
  return [
    ...
    `    volatile bool done;`,
    `    volatile bool ok;`,
    `    TaskHandle_t waiter;   // task waiting on async completion (for the notification handoff)`,
    `    char* resp;`,
    `    size_t resp_len;`,
    `    int64_t content_length;`,
    `    char resp_header[128];`,
    `} __tc_http = { NULL, {0}, HTTP_METHOD_GET, 15000, __TC_HTTP_DEFAULT_MAX_BODY, NULL, 0, false, NULL, {0}, {0}, 0, 0, false, false, NULL, NULL, 0, -1, {0} };`,
    ``,
    ...
    `    __tc_http.body = data;`,
    `    __tc_http.body_len = static_cast<int>(strlen(data));`,
    ...
    `// On completion the worker signals the waiter via a task notification rather`,
    `// than relying solely on the volatile done flag: on dual-core ESP32 the worker`,
    `// can run on CPU1 while app_main polls on CPU0, and volatile gives no cross-`,
    `// core memory-ordering guarantee. xTaskNotifyGive issues a full memory barrier,`,
    `// so once the consumer observes the notification it also observes the`,
    `// resp/status/ok writes the worker made before signaling.`,
    `static void __tc_http_send_task(void* arg) {`,
    `    (void)arg;`,
    `    __tc_http_send();`,
    `    if (__tc_http.waiter) xTaskNotifyGive(__tc_http.waiter);`,
    `    vTaskDelete(NULL);`,
    `}`,
    ``,
    `static inline void __tc_http_send_start(void) {`,
    `    __tc_http.done = false;`,
    `    __tc_http.waiter = xTaskGetCurrentTaskHandle();`,
    `    // Clear any notification left over from a previous request (e.g. the prior`,
    `    // result was observed via the done fast-path before the signal was consumed),`,
    `    // so the first done() poll of this request doesn't fire on a stale signal.`,
    `    ulTaskNotifyTake(pdTRUE, 0);`,
    `    xTaskCreate(__tc_http_send_task, "tc_http", 16384, NULL, 5, NULL);`,
    `}`,
    ``,
    `// Poll predicate: returns true once the worker has notified completion. The`,
    `// ulTaskNotifyTake(pdTRUE, 0) is non-blocking and clears the notification on`,
    `// the first observed signal — its memory barrier makes resp/status/ok safe to`,
    `// read on return.`,
    `static inline bool __tc_http_done(void) {`,
    `    if (__tc_http.done) return true;`,
    `    if (ulTaskNotifyTake(pdTRUE, 0) > 0) { __tc_http.done = true; return true; }`,
    `    return false;`,
    `}`,
    ...
    `static inline long __tc_http_content_length(void) { return static_cast<long>(__tc_http.content_length); }`,
    ``,
