...
export function pulseInitLines(): string[] {
  return [
    ...
    `    while (gpio_get_level((gpio_num_t)pin) != state) { if (static_cast<uint32_t>(esp_timer_get_time() - start) > timeout_us) return 0; }`,
    `    int64_t edge = esp_timer_get_time();`,
    `    while (gpio_get_level((gpio_num_t)pin) == state) { if (static_cast<uint32_t>(esp_timer_get_time() - edge) > timeout_us) return 0; }`,
    `    return static_cast<uint32_t>(esp_timer_get_time() - edge);`,
    `}`,
