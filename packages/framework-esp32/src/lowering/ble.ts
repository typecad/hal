...
export function bleInitLines(): string[] {
  return [
    ...
    `            if (pktlen >= 1) cb_val = ctxt->om->om_data[0];`,
    `        } else if (wt && (strcmp(wt, "uint32") == 0)) {`,
    `            uint32_t u = 0;`,
    `            if (pktlen >= (int)sizeof(u)) memcpy(&u, ctxt->om->om_data, sizeof(u));`,
    `            cb_val = (int)u;`,
    `        } else if (wt && (strcmp(wt, "float32") == 0)) {`,
    `            float f = 0;`,
    `            if (pktlen >= (int)sizeof(f)) memcpy(&f, ctxt->om->om_data, sizeof(f));`,
    `            cb_val = (int)f;`,
    `        } else {`,
    `            // uint8/16, int8/16/32, boolean → int16`,
    `            int16_t s = 0;`,
    `            if (pktlen >= (int)sizeof(s)) memcpy(&s, ctxt->om->om_data, sizeof(s));`,
    `            cb_val = s;`,
    `        }`,
    ...
    `        __tc_ble_char_defs[c].uuid_str = uuid_str;`,
    `        __tc_ble_char_defs[c].uuid_is_128 = __tc_ble_uuid_is_128(uuid_str);`,
    `        __tc_ble_char_defs[c].perms = perms;`,
    `        __tc_ble_char_defs[c].svc_index = svc_index;`,
    `        __tc_ble_char_defs[c].type = type;`,
    `        __tc_ble_char_count = c + 1;`,
    `        __tc_ble.current_char = c;`,
    `    }`,
    `}`,
    ``,
    ...
    `        // Parse 128-bit UUID string into __tc_ble_svc_uuid128_objs[s].`,
    `        ble_uuid128_t* obj = &__tc_ble_svc_uuid128_objs[s];`,
    `        obj->u.type = BLE_UUID_TYPE_128;`,
    `        // Parse "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx" into 16 bytes (big-endian).`,
    `        uint8_t* b = obj->value;`,
    `        int bi = 0;`,
    `        for (int i = 0; u[i] && bi < 16; i++) {`,
    `            if (u[i] == '-') continue;`,
    `            char hex[3] = {u[i], u[i+1], 0};`,
    `            b[bi++] = (uint8_t)strtol(hex, NULL, 16);`,
    ...
    `    (void)idx; (void)value;`,
    `    printf("ble: notify not yet implemented\\n");`,
    `}`,
    ``,
