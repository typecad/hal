...
export function bleInitLines(): string[] {
  return [
    ...
    `typedef void (*__tc_ble_write_cb_t)(void);`,
    ``,
    ...
    `    int16_t char_value[__TC_BLE_MAX_CHARS];   // last-written value per char`,
    `    bool   has_value[__TC_BLE_MAX_CHARS];     // true once a write sets the value`,
    `    char name[32];`,
    ...
    `        int16_t sval;`,
    `        if (__tc_ble.has_value[idx]) {`,
    `            sval = __tc_ble.char_value[idx];  // return last-written value`,
    `        } else if (__tc_ble.on_read[idx]) {`,
    `            sval = (int16_t)__tc_ble.on_read[idx]();  // call read handler`,
    `        } else {`,
    `            sval = 0;`,
    `        }`,
    ...
    `        }`,
    `        __tc_ble.char_value[idx] = val;`,
    `        __tc_ble.has_value[idx] = true;`,
    `        if (__tc_ble.on_write[idx]) __tc_ble.on_write[idx]();`,
    `        return 0;`,
    `    }`,
    `    return 0;`,
    `}`,
