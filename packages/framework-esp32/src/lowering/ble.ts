...
export function bleInitLines(): string[] {
  return [
    ...
    `    // ESP-IDF v5+: nimble_port_init() handles HCI + controller init`,
    `    // (esp_nimble_hci_and_controller_init was removed in v5).`,
    ...
    `}`,
    ``,
    `// TX power: esp_ble_tx_power_set is Bluedroid-only. NimBLE sets TX power`,
       `// via the PHY/controller config at init, not at runtime. No-op for now.`,
