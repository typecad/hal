...
export function bleInitLines(): string[] {
  return [
    ...
    `            const char* s = ((const char*(*)(void))__tc_ble.on_read[idx])();`,
    `            os_mbuf_append(ctxt->om, s, strlen(s) + 1);`,
    ...
    `    (void)idx;  // idx is from the resolver; we auto-assign`,
    ...
    `        __tc_ble_char_defs[c].type = type;  // store for read dispatch`,
    ...
    `                // Pass the char index as arg so the access cb routes to the right handler.`,
