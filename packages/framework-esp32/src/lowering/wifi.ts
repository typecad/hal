...
export function wifiInitLines(): string[] {
  return [
    ...
    `    bool ap_started;`,
    `    bool auto_reconnect;`,
    `    bool pending_disconnect;  // set by disconnect(); honored once by the STA_DISCONNECTED handler`,
    ...
    `} __tc_wifi = { 0, false, false, false, false, false, false, true, false, NULL, NULL, {0}, {0}, 1, false, 4, -1, 0, {}, NULL, NULL, NULL };`,
    ``,
    ...
    `            }`,
    `            // An explicit disconnect() sets pending_disconnect so this event`,
    `            // (which arrives asynchronously after esp_wifi_disconnect returns)`,
    `            // does not immediately trigger auto_reconnect and undo the call.`,
    `            // The flag is consumed exactly once here.`,
    `            if (__tc_wifi.pending_disconnect) {`,
    `                __tc_wifi.pending_disconnect = false;`,
    `            } else if (__tc_wifi.auto_reconnect && __tc_wifi.sta_started) {`,
    `                esp_wifi_connect();`,
    ...
    `    ESP_LOGI("tc", "wifi: init (%s)", ssid ? ssid : "");`,
    ...
    `        ESP_LOGI("tc", "wifi: starting radio");`,
    `        __tc_wifi_start_radio();`,
    `        __tc_wifi.sta_started = true;`,
    `    } else {`,
    `        __tc_wifi_apply_tx_power();`,
    `    }`,
    `    ESP_LOGI("tc", "wifi: connecting");`,
    ...
    `    // Mark the disconnect as user-initiated. esp_wifi_disconnect() returns before`,
    `    // WIFI_EVENT_STA_DISCONNECTED is processed, so the event handler (which runs`,
    `    // later on the event loop) would otherwise see auto_reconnect still set and`,
    `    // immediately call esp_wifi_connect(), undoing the disconnect. The handler`,
    `    // consumes pending_disconnect once. (We no longer toggle auto_reconnect`,
    `    // synchronously, since restoring it before the event fires reintroduces the race.)`,
    `    __tc_wifi.pending_disconnect = true;`,
    `    esp_wifi_disconnect();`,
    `    __tc_wifi.status = 4;`,
    `    __tc_wifi.got_ip = false;`,
    `}`,
    ...
    `    if (dbm < 2) dbm = 2;`,
    `    if (dbm > 20) dbm = 20;`,
    `    __tc_wifi.tx_power_qdbm = static_cast<int8_t>(dbm * 4);`,
    ...
    `    wc.ap.ssid_len = static_cast<uint8_t>(strlen(ssid));`,
