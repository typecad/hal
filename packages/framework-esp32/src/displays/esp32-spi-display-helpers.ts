...
export function esp32SpiCmdDataHelpers(): string {
  return [
    `// Send a single command byte (DC low). Matches Adafruit_SPITFT::writeCommand`,
    `// exactly: DC LOW → send byte → DC HIGH (restore data mode) → CS HIGH.`,
    `// Restoring DC to data-mode before raising CS is more faithful to the`,
    `// ST7796S command-latch protocol than leaving DC low at CS-rise.`,
    ...
    `  gpio_set_level((gpio_num_t)__esp32_display.dc_pin, 1);  // restore data mode`,
...
export function esp32SpiPanelOps(addrWindowCmds: "ili9341" | "st7796"): string {
  ...
  return [
    ...
    `  uint32_t xs = static_cast<uint32_t>(x), xe = static_cast<uint32_t>(x + w - 1);`,
    `  uint32_t ys = static_cast<uint32_t>(y), ye = static_cast<uint32_t>(y + h - 1);`,
    `  uint8_t col[5]  = { ${colCmd}, static_cast<uint8_t>(xs >> 8), static_cast<uint8_t>(xs & 0xFF), static_cast<uint8_t>(xe >> 8), static_cast<uint8_t>(xe & 0xFF) };`,
    `  uint8_t page[5] = { ${pageCmd}, static_cast<uint8_t>(ys >> 8), static_cast<uint8_t>(ys & 0xFF), static_cast<uint8_t>(ye >> 8), static_cast<uint8_t>(ye & 0xFF) };`,
    ...
    `      buf[i * 2]     = static_cast<uint8_t>(*p >> 8);    // high byte first`,
    `      buf[i * 2 + 1] = static_cast<uint8_t>(*p & 0xFF);`,
    `      p++;`,
    `    }`,
    `    spi_transaction_t t = {};`,
    `    t.length = chunk * 16;`,
    ...
    `  spi_device_polling_transmit(__esp32_display.dev, &cmd_t);`,
    `  gpio_set_level((gpio_num_t)__esp32_display.dc_pin, 1);`,
    `  // RGB565 big-endian on the wire: high byte first, then low byte.`,
    `  // Extract from the ORIGINAL color value — do NOT bswap first. bswap`,
    `  // reverses the VALUE, and extracting high/low bytes from the reversed`,
    `  // value double-swaps (the previous bug: red 0xF800 became 0x00F8 green).`,
    `  uint8_t pix[2] = { static_cast<uint8_t>(c >> 8), static_cast<uint8_t>(c & 0xFF) };`,
    `  spi_transaction_t t = {};`,
    `  t.length = 16;`,
    `  t.tx_buffer = pix;`,
    `  spi_device_polling_transmit(__esp32_display.dev, &t);`,
    ...
    `  spi_device_polling_transmit(__esp32_display.dev, &cmd_t);`,
    `  gpio_set_level((gpio_num_t)__esp32_display.dc_pin, 1);`,
    `  // RGB565 big-endian on the wire. Extract high/low from the ORIGINAL color`,
    `  // value — do NOT bswap. (Previous bug: bswap16(c) reversed the value, then`,
    `  // extracting high/low from the reversed value produced double-swapped`,
    `  // bytes, so red 0xF800 became 0x00F8 dark green on the panel.)`,
    `  uint8_t buf[2048];`,
    `  for (uint16_t i = 0; i < 1024; i++) {`,
    `    buf[i * 2]     = static_cast<uint8_t>(c >> 8);    // high byte first`,
    `    buf[i * 2 + 1] = static_cast<uint8_t>(c & 0xFF);  // low byte`,
    `  }`,
    `  while (n > 0) {`,
...
export function esp32SpiAdapterSurface(): string {
  return [
    ...
    `}`,
    `static inline void display_targetFillRect(CuttlefishDisplayTarget* t, int16_t x, int16_t y, int16_t w, int16_t h, UI_COLOR_T color) { t->fillRect(x, y, w, h, static_cast<uint16_t>(color)); }`,
    `static inline void display_targetDrawFastHLine(CuttlefishDisplayTarget* t, int16_t x, int16_t y, int16_t w, UI_COLOR_T color) { t->drawFastHLine(x, y, w, static_cast<uint16_t>(color)); }`,
    `static inline void display_targetDrawFastVLine(CuttlefishDisplayTarget* t, int16_t x, int16_t y, int16_t h, UI_COLOR_T color) { t->drawFastVLine(x, y, h, static_cast<uint16_t>(color)); }`,
    `static inline void display_targetFillRoundRect(CuttlefishDisplayTarget* t, int16_t x, int16_t y, int16_t w, int16_t h, int16_t r, UI_COLOR_T color) { t->fillRoundRect(x, y, w, h, r, static_cast<uint16_t>(color)); }`,
    `static inline void display_targetDrawRect(CuttlefishDisplayTarget* t, int16_t x, int16_t y, int16_t w, int16_t h, UI_COLOR_T color) { t->drawRect(x, y, w, h, static_cast<uint16_t>(color)); }`,
    `static inline void display_targetDrawRoundRect(CuttlefishDisplayTarget* t, int16_t x, int16_t y, int16_t w, int16_t h, int16_t r, UI_COLOR_T color) { t->drawRoundRect(x, y, w, h, r, static_cast<uint16_t>(color)); }`,
