...
export function esp32SpiCmdDataHelpers(): string {
  return [
    ...
    `}`,
    ``,
    `// Send a command followed by N data bytes (DC transitions once).`,
    `static inline void __esp32_spi_cmd_data(const uint8_t* bytes, uint8_t n) {`,
    `  // First byte is the command; remaining are data.`,
    `  __esp32_spi_cmd(bytes[0]);`,
    `  gpio_set_level((gpio_num_t)__esp32_display.dc_pin, 1);`,
    `  gpio_set_level((gpio_num_t)__esp32_display.cs_pin, 0);`,
...
export function esp32SpiPanelOps(addrWindowCmds: "ili9341" | "st7796"): string {
  ...
  return [
    ...
    `  // spi_device_polling_transmit handles up to 4096 bytes per call; chunk if needed.`,
    `  gpio_set_level((gpio_num_t)__esp32_display.cs_pin, 0);`,
    ...
    `  uint16_t swapped = (c >> 8) | (c << 8);  // big-endian on wire`,
    `  gpio_set_level((gpio_num_t)__esp32_display.cs_pin, 0);`,
    `  spi_transaction_t t = {};`,
    `  t.length = 16;`,
    `  t.tx_buffer = &swapped;`,
