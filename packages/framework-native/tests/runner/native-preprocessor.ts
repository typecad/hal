...
import { isDescribeChain, collectChainSegments } from '@typecode/expect/chain-collector';
import type { ChainSegment } from '@typecode/expect/chain-collector';
...
export function nativePreprocess(source: string, fileName: string = 'test.ts'): string {
  ...
  for (const stmt of sf.statements) {
    if (ts.isImportDeclaration(stmt)) {
      ...
      if (mod === '@typecode/expect') continue;
