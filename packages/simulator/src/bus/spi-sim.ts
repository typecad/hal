...
export class SimSPIBus implements ISPIBus {
  ...
  device(chipSelect: IDigitalPin): ISPIDevice {
   ...
   */
  attachDevice(csPin: IDigitalPin, device: ISimSPIDevice): void {
   ...
   */
  detachDevice(csPin: IDigitalPin): void {
...
class SimSPIDevice implements ISPIDevice {
  ...
  constructor(bus: SimSPIBus, chipSelect: IDigitalPin) {
    this.bus = bus;
    this._simBus = bus;
