...
// ---------------------------------------------------------------------------
// Internal — Serial Initialization
// ---------------------------------------------------------------------------

/**
 * Generate Serial.begin() initialization code for Arduino.
 */
...
function generateBreakpointCode(
  ...
  if (bp.logMessage) {
    return generateLogpointCode(fileName, lineNum, bp.logMessage, variables);
  }
 ...
 * Logpoints use {variable} syntax for interpolation.
 */
function generateLogpointCode(
  ...
  // Parse the log message for {variable} interpolation
  const parts = parseLogMessage(logMessage);
