// ---------------------------------------------------------------------------
// ArduinoCliToolchain — toolchain implementation for arduino-cli
//
 ...
 * Arduino requires all code in a single .ino or properly organized subdirectory.
 */
...
export class ArduinoCliToolchain implements Toolchain {
  ...
  async compile(options: CompileOptions): Promise<CompileResult> {
    ...
    // Handle hidden directories (e.g., .typecode)
    ...
    if (
      ...
      try {
        ...
        // Remove other .ino files to avoid duplicate setup()/loop()
    ...
    if (options.optimize === 'size') {
      // arduino-cli doesn't have direct size optimization flag
      // It uses -Os by default for AVR
    }
  ...
  async listPorts(): Promise<Array<{ port: string; description?: string }>> {
    ...
    try {
      ...
      // arduino-cli board list returns an array of port objects
...
// Also export the legacy functions for backward compatibility
