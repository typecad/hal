...
it auto-activates the micromamba env created by `packages/zephyr-installer`.
...
```sh
cp -r packages/zephyr-installer/templates/project/.typecad ./
mkdir -p .vscode
cp    packages/zephyr-installer/templates/project/.vscode/settings.json ./.vscode/settings.json
```
