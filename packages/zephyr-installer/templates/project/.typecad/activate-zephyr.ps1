...
#
# Activates the micromamba env created by packages/zephyr-installer. After
...
if (-not (Get-Command micromamba -ErrorAction SilentlyContinue)) {
  ...
  if (-not (Test-Path $mm)) {
    throw "micromamba not found at $mm. Run the typeCAD Zephyr installer first: node packages/zephyr-installer/install.mjs"
  }
