...
```sh
npx @typecad/zephyr-installer                        # install (interactive checklist)
npx @typecad/zephyr-installer --platforms arm,esp32   # install (non-interactive)
npx @typecad/zephyr-installer --modify                # add/remove platform toolchains
npx @typecad/zephyr-installer --delete                # uninstall everything
npx @typecad/zephyr-installer --help                  # full usage reference
npx @typecad/zephyr-installer --dry-run               # preview the resolved plan
```

(From a repo checkout, `node packages/zephyr-installer/install.mjs` is the same
...
- **`npx @typecad/zephyr-installer` does nothing** — fixed in alpha.11 (the
  entry-point guard now follows symlinks). If on an older alpha, run
  `node node_modules/@typecad/zephyr-installer/install.mjs` directly.
