...
export function wifiInitLines(): string[] {
  return [
    ...
    `static void __tc_wifi_mgmt_cb(struct net_mgmt_event_callback* cbb, int32_t mgmt_event,`,
    `                              struct net_if* iface) {`,
    `    (void)cbb;`,
    `    (void)iface;`,
    ...
    `        const struct wifi_scan_result* res = (const struct wifi_scan_result*)cbb->info;`,
    ...
    `    net_mgmt_init_event_handler(&__tc_wifi_l4_cb, __tc_wifi_mgmt_cb);`,
    `    net_mgmt_add_event_callback(&__tc_wifi_l4_cb,`,
    `        NET_EVENT_L4_CONNECTED | NET_EVENT_L4_DISCONNECTED);`,
    `    net_mgmt_init_event_handler(&__tc_wifi_scan_cb, __tc_wifi_mgmt_cb);`,
    ...
    `// SSID/password are staged before connect via the WiFi-specific connect`,
    `// request (NET_REQUEST_WIFI_CONNECT with wifi_connect_req_params); conn_mgr`,
    `// then brings the iface up and the L4 handler signals end-to-end connectivity.`,
    ...
    `    for (uint32_t b = 0; b < ssid_len; b++) { __tc_wifi_conn_params.ssid[b] = static_cast<uint8_t>(ssid[b]); }`,
    `    __tc_wifi_conn_params.ssid_length = ssid_len;`,
    `    if (password != nullptr) {`,
    `        uint32_t pw_len = strlen(password);`,
    `        if (pw_len > 63U) pw_len = 63U;`,
    `        for (uint32_t b = 0; b < pw_len; b++) { __tc_wifi_conn_params.password[b] = static_cast<uint8_t>(password[b]); }`,
    `        __tc_wifi_conn_params.password_length = pw_len;`,
    `    } else {`,
    `        __tc_wifi_conn_params.password_length = 0U;`,
    `    }`,
    ...
    `    struct in_addr* addr = &__tc_wifi.iface->config.ip.ipv4->unicast[0].address.in_addr;`,
    ...
    `    int32_t ret = net_mgmt(NET_REQUEST_WIFI_RSSI, __tc_wifi.iface, &status, sizeof(status));`,
    `    if (ret != 0) return 0;`,
    ...
    `    __tc_wifi_ensure_init();`,
    `    struct wifi_iface_status status = { 0 };`,
    `    (void)net_mgmt(NET_REQUEST_WIFI_RSSI, __tc_wifi.iface, &status, sizeof(status));`,
    `    // Pack the 6 bytes into a uint64 (HAL returns the MAC as a number).`,
    `    uint64_t m = 0;`,
    `    for (int32_t b = 0; b < 6; b++) { m = (m << 8) | static_cast<uint64_t>(status.mac_addr[b]); }`,
    `    return m;`,
    `}`,
    ``,
    ...
    `    __tc_wifi_ensure_init();`,
    `    net_if_set_hostname(__tc_wifi.iface, name);`,
    `}`,
    `static void __tc_wifi_set_tx_power(int32_t dbm) {`,
    `    __tc_wifi_ensure_init();`,
    `    int32_t power = dbm;`,
    `    (void)net_mgmt(NET_REQUEST_WIFI_TX_POWER, __tc_wifi.iface, &power, sizeof(power));`,
    `}`,
    ``,
    `// CUTTLEFISH_WIFI_END`,
  ];
