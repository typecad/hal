...
export function lowerPulseOrShift(
  ...
  const o = op as any;
  const controller = chip.gpioController;
  ...
  switch (op.operation) {
    case 'pulse.in': {
      ...
      return {
        expression: `({ int64_t __max = static_cast<int64_t>(${timeout} / 1000); int64_t __t0 = k_uptime_get(); bool __ok = true; while (gpio_pin_get_raw(DEVICE_DT_GET(DT_NODELABEL(${controller})), ${pin}) != ${want}) { if ((k_uptime_get() - __t0) > __max) { __ok = false; break; } } int32_t __ret = 0; if (__ok) { int64_t __start = k_uptime_get(); while (gpio_pin_get_raw(DEVICE_DT_GET(DT_NODELABEL(${controller})), ${pin}) == ${want}) { } __ret = static_cast<int32_t>((k_uptime_get() - __start) * 1000); } __ret; })`,
      };
    }
    case 'pulse.in_long': {
    ...
    case 'shift.out': {
      // Bit-bang `value` (MSB or LSB first) via dataPin + clockPin.
      ...
      return {
        code: `for (int __i = ${init}; ${test}; ${step}) { gpio_pin_set_raw(DEVICE_DT_GET(DT_NODELABEL(${controller})), ${dataPin}, (${o.value} >> __i) & 1); gpio_pin_set_raw(DEVICE_DT_GET(DT_NODELABEL(${controller})), ${clockPin}, 1); k_busy_wait(1); gpio_pin_set_raw(DEVICE_DT_GET(DT_NODELABEL(${controller})), ${clockPin}, 0); }`,
      };
    }
    case 'shift.in': {
      ...
      return {
        expression: `({ uint8_t __v = 0; for (int __i = ${init}; ${test}; ${step}) { gpio_pin_set_raw(DEVICE_DT_GET(DT_NODELABEL(${controller})), ${clockPin}, 1); k_busy_wait(1); int bit = gpio_pin_get_raw(DEVICE_DT_GET(DT_NODELABEL(${controller})), ${dataPin}); gpio_pin_set_raw(DEVICE_DT_GET(DT_NODELABEL(${controller})), ${clockPin}, 0); ${accum}; } __v; })`,
      };
    }
    default:
      throw new Error(
