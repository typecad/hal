...
describe("Preferences namespace")
  ...
  .it("Preferences.putUInt then Preferences.getUInt roundtrip")
  .expect(
    (() => {
      Preferences.begin("test");
      Preferences.putUInt("counter", 1000);
      const v = Preferences.getUInt("counter", 0);
      Preferences.end();
      return v;
    })
  ).toBe(1000)
  .it("Preferences.putFloat then Preferences.getFloat roundtrip")
  .expect(
    (() => {
      Preferences.begin("test");
      Preferences.putFloat("gain", 2.5);
      const v = Preferences.getFloat("gain", 0);
      Preferences.end();
      return (v > 2.4 && v < 2.6) ? 1 : 0;
    })
  ).toBe(1)
