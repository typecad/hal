...
// Shared board-level SPI suite. The chip-select pin comes from the board
// package's test-pins.json; everything else uses the SPI0 bus singleton.
...
describe("SPI0 device accessor")
  ...
  .expect(
    (() => {
      SPI0.device(CS_PIN).transfer(0x55);
      return 1;
    })
  ).toBe(1)
  ...
  .expect(
    (() => {
      SPI0.device(CS_PIN).writeRegister(0x00, 0x01);
      return 1;
    })
  ).toBe(1)
  ...
  .expect(
    (() => {
      SPI0.device(CS_PIN).readRegister(0x00, 1);
      return 1;
    })
  ).toBe(1)
