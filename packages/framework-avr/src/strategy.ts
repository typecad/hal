  ...
  override shimLines(program?: ProgramIR, ctx?: PlatformContext): string[] {
    ...
    if (usesUART) {
      lines.push(
        ...
        '  uint16_t ubrr;',
        '  UCSR0A = (1 << U2X0);',
        '  ubrr = static_cast<uint16_t>((F_CPU / 4 / baud - 1) / 2);',
        '  if (ubrr > 4095) {',
        '    UCSR0A = 0;',
        '    ubrr = static_cast<uint16_t>((F_CPU / 8 / baud - 1) / 2);',
        '  }',
        ...
        '  long integer = static_cast<long>(num);',
        '  long decimal = static_cast<long>((num - integer) * 100);',
    ...
    if (analysis ? (analysis.usesNativeTiming || analysis.usesI2C || usesUART) : true) {
      lines.push(
        ...
        '    uint16_t loops = static_cast<uint16_t>(((F_CPU / 1000000UL) * chunk) / 4UL);',
    ...
    if (usesUART) {
      lines.push(
        ...
        '  _uart_print_long(static_cast<long>(val));',
        '}',
    ...
    if (analysis ? analysis.usesI2C : true) {
      ...
      lines.push(
        ...
        '  return static_cast<int>(_twi_rx_head - _twi_rx_tail);',
        '}',
    ...
    if (analysis ? analysis.usesTone : true) {
      lines.push(
        ...
        '    _tc_tone_toggle_count = static_cast<long>((2UL * freq * duration) / 1000UL);',
  ...
  override resolveHALOperation(op: HALOpIR): { code?: string; expression?: string } | undefined {
    switch (op.operation) {
      ...
      case "adc.read_voltage": {
        ...
        return { expression: `(static_cast<double>(${nativeAnalogRead(op.pin)}) * ${vRef} / ${maxVal})` };
      }
