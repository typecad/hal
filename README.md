...
- **Runtime tracing** — `TypeCAD: Trace — Capture from Board` runs `typecad-hal trace capture` in the shared terminal with the session's serial port and opens the live timeline panel (per-thread CPU lanes, the UI frame line + tick-phase bars, `Trace.mark`/`Trace.event` markers) — the panel follows the capture as it writes, via a file watcher. `Trace — View Timeline` reopens it, `Trace — Show Report` prints the summary, and `Trace — Enable in Project` inserts the `zephyr.trace` record into `typecad-hal.config.ts` (a capture against an untraced build would see no heartbeats).
...
typecad-hal trace capture [--flash] [--gates-file <f>] [--port <p>] [--duration <s>|--forever]
                                                   # one command: (re)flash, preflight the build, record [TR: heartbeats,
