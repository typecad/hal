...
export const consolePolyfill: PolyfillDefinition = {
  ...
  generate(needs: PolyfillNeed[], context: PolyfillContext): RuntimePolyfillIR {
    ...
    if (impl === "serial") {
      return generateArduinoConsolePolyfill(methods, isAvr, context.config?.console?.useFlashStrings ?? true);
    } else {
...
`);
  ...
  shimMacros.push(`// Note: Add Serial.begin(9600); in setup() for console output`);
